...
df$shares75 = as.factor(ifelse(df$shares > quantile(df$shares, 0.75), 1, 0))
df[,c('shares', 'url', 'timedelta', 'LDA_00', 'LDA_01', 'LDA_02', 'LDA_03', 'LDA_04')] = list(NULL)

# X/y, train-test split
train = createDataPartition(df$shares75, p=0.66, list=F)
...
model.glmnet = train(Xtrain.sc, ytrain,
                     ...
                     metric='Accuracy',
                     maximize=T)
save_model_sc(model.glmnet, name='glmnet75')
...
model.knn = train(Xtrain.sc, ytrain,
                  ...
                  metric='Accuracy',
                  maximize=T)
save_model_sc(model.knn, name='knn75')
