...
wiki = na.omit(wiki)
wiki$row.names = NULL # if it's null why is it still there??? check out
...
```
\begin{center}\textbf{Figure 2.5 - Hierarchical clustering}\end{center}
\hrulefill
...
print(formula)
cm.calc(lda)
# results are really good so the assumptions (conditional normality or something - look up) must be true
```
...
print(formula)
cm.calc(qda)
# predicts a ton of 0s which leads to high precision and low recall
# but actually also has no false positives?
```
...
cm.knn(df.train.sc, df.test.sc, 4) # probably the best
